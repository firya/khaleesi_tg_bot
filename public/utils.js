...
export const parseDate = (strDate, UTC = false) => {
  ...
  if (strDate.indexOf("-") !== -1) {
    ...
    if (dateArray.length == 3) {
      ...
      to = dateToTimestamp(
        `${leftPad(toDate.getDate(), 2)}.${leftPad(
          toDate.getMonth() + 1,
          2
        )}.${toDate.getFullYear()}`,
        UTC,
...
export const dateForRoistat = (date) => {
  ...
  return `${date.getFullYear()}-${leftPad(date.getMonth() + 1, 2)}-${leftPad(
    date.getDate(),
    2
  )}T${leftPad(date.getHours(), 2)}:${leftPad(date.getMinutes(), 2)}:${leftPad(
    date.getSeconds(),
    2
  )}+0000`;
};
...
export const dateToTimestamp = (strDate, UTC = false, to = false) => {
  ...
  let date = to
    ? `${dateArray[1]}/${dateArray[0]}/${dateArray[2]} 23:59:59`
    : `${dateArray[1]}/${dateArray[0]}/${dateArray[2]} 00:00:00`;

  var result = !UTC
    ? (Date.parse(date) - timeZoneOffset * 1000) / 1000
...
export const dateIntervalToUserView = (
  interval,
  prefix = "Отчет за",
  mark = true,
  UTC = false
  ...
  if (from.date == to.date) {
    return mark
      ? `${prefix} ${from.date}, ${from.dow} ${from.mark}`
      : `${prefix} ${from.date}, ${from.dow}`;
  } else {
...
export const findDateAlias = (date) => {
  ...
  const aliasList = {
    today: `${leftPad(today.getDate(), 2)}.${leftPad(
      today.getMonth() + 1,
      2
    )}.${today.getFullYear()}`,
    yesterday: `${leftPad(yesterday.getDate(), 2)}.${leftPad(
...
export const joinReplayByMaxLength = (replayArray, maxLength = 4096) => {
  ...
  replayArray.map((replay) => {
    if (
      !result.length ||
      result[result.length - 1].length + replay.length > maxLength - 4
    ) {
      result.push("");
