...
export const defaultKeyboard = (admin = false) => {
  if (admin) {
    return [
      ...
      ["📏 Назначенные"],
      ["🚪 Закрытые"],
      ["⏱ Добавить отчет", "❌ Удалить отчет"],
      ["🙎‍♂️ Add User", "💀 Remove User"],
      ["📌 Отслеживание", "⏳ Интервал"],
      ["/help"],
    ];
  } else {
    return [
...
const generateFewIntervals = (props = {}) => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    result.push({
      text: textList[i]
        ? textList[i]
        : `${leftPad(fromDay, 2)}.${leftPad(
            fromMonth + 1,
            2
          )}.${fromYear}-${leftPad(day, 2)}.${leftPad(month + 1, 2)}.${year}`,
      callback_data: `${props.callback_data} ${leftPad(fromDay, 2)}.${leftPad(
        fromMonth + 1,
        2
      )}.${fromYear}-${leftPad(day, 2)}.${leftPad(month + 1, 2)}.${year}`,
    });
...
const generateFewDays = (props = {}) => {
  ...
  for (let i = 0; i < props.count; i++) {
    ...
    result.push({
      text: textList[i]
        ? `${textList[i]}`
        : `${leftPad(day, 2)}.${leftPad(month + 1, 2)}.${year}`,
      callback_data: `${props.callback_data} ${leftPad(day, 2)}.${leftPad(
        month + 1,
        2
      )}.${year}`,
    });
